# educational-robotics-projects

Este repositório contém meus projetos e atividades relacionados à **Robótica Educacional**, desenvolvidos como parte da minha trajetória de aprendizado e estudos acadêmicos.

## 🎯 Objetivo
O objetivo deste repositório é documentar meu processo de aprendizagem e compartilhar códigos e experimentos utilizados em aulas de robótica, com foco na **educação STEM** (Ciência, Tecnologia, Engenharia e Matemática).

## 🛠️ Ferramentas e Tecnologias
- **Hardware:** Arduino, sensores  
- **Linguagens:** C++ 
- **Software:** Arduino IDE, Tinkercad  

## 📂 Estrutura do Repositório
- `/projects` – Projetos completos de robôs, incluindo código e esquemas   

## 📚 Contexto Acadêmico
Alguns dos projetos deste repositório foram desenvolvidos no contexto de disciplinas cursadas durante o curso de pós-graduação em Inovação, Inteligência Artificial e Robótica Educacional da Gran Faculdade.
Todo o conteúdo é disponibilizado **exclusivamente para fins educacionais e de aprendizagem**.

---
Criado por **Antony**
